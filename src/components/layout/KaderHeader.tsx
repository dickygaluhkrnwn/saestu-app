...
export function KaderHeader() {
  const { userProfile, logout } = useAuth();
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);

  // Menutup menu jika klik di luar kotak dropdown (Native App Behavior)
  useEffect(() => {
    ...
    <div className="md:hidden sticky top-0 z-50 flex justify-center print:hidden pointer-events-none">
      ...
      <header className="w-full bg-white/90 backdrop-blur-xl border-b border-slate-200/50 px-5 py-3.5 flex justify-between items-center pointer-events-auto relative shadow-sm">
        ...
        <div className="flex items-center gap-2.5">
          <div className="bg-gradient-to-br from-blue-500 to-indigo-600 p-1.5 rounded-[0.7rem] shadow-sm shadow-blue-500/20">
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <div className="w-7 h-7 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-[10px] font-black">
        ...
        {isMenuOpen && (
          ...
          >
            <div className="px-3 py-2 border-b border-slate-50 mb-1">
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Menu Kader</p>
            </div>
            
            <Link href="/posyandu/profile" onClick={() => setIsMenuOpen(false)} className="flex items-center gap-3 p-2.5 hover:bg-slate-50 rounded-xl transition-colors group">
